#  Adapted from https://github.com/PriorLabs/TabPFN/blob/main/src/tabpfn/architectures/base/transformer.py
...
if TYPE_CHECKING:
    from explainerpfn.model.config import ModelConfig
...
class PerFeatureTransformer(nn.Module):
    ...
    def forward(  # noqa: PLR0912, C901
        ...
        x: torch.Tensor | dict[str, torch.Tensor],
        y: torch.Tensor | dict[str, torch.Tensor] | None,
        single_eval_pos: int,  # TODO: check if this is correct
        *,
        ...
        # single_eval_pos = y["main"].shape[0]  # TODO: check if this is correct
        ...
        for k in y:
            ...
            if y[k].shape[1] < x["main"].shape[1]:
                ...
                # TODO: This should be raised as an error on the first working run
                # TODO: Remove this after the first working run
